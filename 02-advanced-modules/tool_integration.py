...
@app.cell
def __(available_providers, cleandoc, mo, output, time):
    if available_providers:
        cell3_desc = mo.md(
            cleandoc(
                """
            ...
            def get_tool_info(self, name: str = None):
                ...
                if name:
                    ...
                    return {
                        ...
                        "total_calls": sum(
                            s["calls"] for s in self.execution_stats.values()
                        ),
                    }
...
@app.cell
def __(available_providers, cleandoc, mo, output, time, tool_registry):
    if available_providers and tool_registry:
        ...
        # Mathematical Tools
        def calculator_tool(calc_inputs: dict, context: dict = None) -> dict:
            """Advanced calculator with multiple operations."""
            expression = calc_inputs.get("expression", "")
            operation = calc_inputs.get("operation", "evaluate")
            ...
            try:
                if operation == "evaluate":
                    ...
                    numbers = calc_inputs.get("numbers", [])
                    if not numbers:
        ...
        def text_processor_tool(txt_process_inputs: dict, context: dict = None) -> dict:
            """Advanced text processing capabilities."""
            text = txt_process_inputs.get("text", "")
            operation = txt_process_inputs.get("operation", "analyze")
            ...
            if operation == "analyze":
                ...
                transform_type = txt_process_inputs.get("transform_type", "uppercase")
                ...
                extract_type = txt_process_inputs.get("extract_type", "words")
        ...
        def web_search_tool(web_search_inputs: dict, context: dict = None) -> dict:
            """Simulated web search tool (in production, integrate with real search APIs)."""
            query = web_search_inputs.get("query", "")
            max_results = web_search_inputs.get("max_results", 5)
            ...
            search_database = {
                "python": [
                    ...
                    },
                    {
                        ...
                        "snippet": "An informal introduction to Python programming language.",
                    },
                    {
                        "title": "Real Python",
                        "url": "https://realpython.com",
                        "snippet": "Python tutorials and resources for developers.",
                    },
                ],
                "machine learning": [
                    {
                    ...
                    },
                    {
                        ...
                        "snippet": "An end-to-end open source platform for machine learning.",
                    },
                ],
                "dspy": [
                    {
                        ...
                        "url": "https://dspy.ai",
                        "snippet": "DSPy is a framework for algorithmically optimizing LM prompts and weights.",
                    },
                    {
                        "title": "DSPy Documentation",
                        "url": "https://dspy.ai/docs",
                        "snippet": "Comprehensive guide to using DSPy for LM programming.",
                    },
                ],
            }
                ...
                results = [
                    {
                        ...
                        "snippet": f"No specific results found for '{query}' in simulated database.",
                    }
                ]
        ...
        # Data Storage Tool
        def data_storage_tool(data_storage_inputs: dict, context: dict = None) -> dict:
            """Simple data storage and retrieval tool."""
            operation = data_storage_inputs.get("operation", "store")

            # Initialize or reset storage if not exists
            if not hasattr(data_storage_tool, "storage") or operation == "reset":
                data_storage_tool.storage = {}
                if operation == "reset":
                    return {"message": "Storage cleared successfully"}

            if operation == "store":
                key = data_storage_inputs.get("key", "")
                value = data_storage_inputs.get("value", "")
                if not key:
        ...
        tool_registry.register_tool(
            ...
            "Text analysis, transformation, and extraction capabilities",
            {
                ...
                "optional": ["transform_type", "extract_type"],
            },
...
def __(available_providers, cleandoc, dspy, mo, output):
    if available_providers:
        ...
        class ToolIntegratedAgentSignature(dspy.Signature):
            ...
            reasoning = dspy.OutputField(
                desc="Step-by-step reasoning about the problem"
            )
            tool_plan = dspy.OutputField(
                desc="Plan for which tools to use and in what order"
            )
            tool_calls = dspy.OutputField(
                desc="Specific tool calls with inputs (JSON format)"
            )
            expected_outcome = dspy.OutputField(
                desc="Expected outcome from tool execution"
            )
    ...
    return (
        ToolIntegratedAgentSignature,
        tool_agent,
    )


@app.cell
def __(available_providers, mo, output, tool_registry):
    if available_providers and tool_registry:
        ...
        run_agent_demo = mo.ui.run_button(label="🤖 Run Tool-Integrated Agent")
        clear_cache_button = mo.ui.run_button(label="🗑️ Clear Tool Cache")
        ...
        cell6_out = mo.vstack(
            [
                ...
                mo.hstack([run_agent_demo, clear_cache_button]),
            ]
        )
    else:
...
def __(
    ...
    if available_providers and clear_cache_button and clear_cache_button.value:
        # Clear the data storage tool cache
        cache_clear_result = tool_registry.execute_tool(
            "data_storage", {"operation": "reset"}
        )
        cell7_out = mo.md(
            f"✅ **Cache Cleared:** {cache_clear_result.get('result', {}).get('message', 'Storage cleared')}"
        )
    elif available_providers and run_agent_demo.value and problem_selector.value:
        try:
            selected_problem = problem_selector.value

            # Get agent's plan
            agent_response = tool_agent(
                problem=selected_problem, available_tools=available_tools_desc
            )
            ...
            try:
                ...
                if (
                    "text" in tool_calls_text.lower()
                    or "analyze" in tool_calls_text.lower()
                ):
                    # Text analysis
                ...
                tool_results.append(
                    ("error", {"error": f"Tool execution error: {str(e)}"})
                )

            # Display comprehensive results
            tool_result_displays = []
            for tool_name, result in tool_results:
                tool_result_displays.append(
                    cleandoc(
                        f"""
            ...
            cell7_out = mo.vstack(
                [
                    ...
                    mo.md(
                        "\n".join(tool_result_displays)
                        if tool_result_displays
                    ...
                    mo.md(
                        f"**Execution Success:** {'✅ Yes' if all(r[1].get('success', True) for r in tool_results) else '⚠️ Partial'}"
                    ),
                ]
            )

        except Exception as e:
            cell7_out = mo.md(f"❌ **Agent Demo Error:** {str(e)}")
    else:
        cell7_out = mo.md(
            "*Select a problem and click 'Run Tool-Integrated Agent' to see the agent in action, or click 'Clear Tool Cache' to reset storage*"
        )
